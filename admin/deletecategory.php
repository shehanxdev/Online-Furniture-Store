...
<section class="right"><!--new section on the right is open here  -->
	...
	<?php
		...
		if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) { /*to login */
			...
			$products = $pdo->query('DELETE FROM category WHERE id = ' . $_POST['id']); /* selecting from the table*/

			echo 'Category deleted';  /*to delete the category */

		}

		else { /*if the user is able to login it comes back to the login form  */
			?>
			<h2>Log in</h2>
