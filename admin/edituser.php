...
if (isset($_POST['submit'])) {
    $query = 'UPDATE user SET name = :name';  /*to update the name */ 
    $criteria = [
        'name' => $_POST['name'], /*criteria for the name */ 
        'id' => $_POST['id'],   /*criteria for the id */ 
    ];
    if (isset($_POST['password'])) {
        $query .= ', password = :password';
        $criteria['password'] = password_hash($_POST['password'], PASSWORD_DEFAULT);  /* for hash password */
    }
    $query .= ' WHERE id = :id';  /*query  for the id */ 
    $stmt = $pdo->prepare($query);
    $stmt->execute($criteria);  /*to execute the criteria */ 
    echo 'User Saved';  /*when the user is updated  */ 
} else {
    if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
        $currentUser = $pdo->query('SELECT * FROM user WHERE id = ' . $_GET['id'])->fetch();  /*to fetch the data from the table*/ 
        ?>
