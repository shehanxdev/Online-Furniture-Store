...
<html><!--this line of code starts with html tag -->
	...
	<nav><!--here we start with the nav tag -->
		<ul><!--all links are added under this tag -->
			...
			<li><a href="/contact.php">Contact us</a></li><!--this  link will open the contacts page -->
			<li><a href="/admin/logout.php">Logout</a></li>
</ul><!--here we close the ul tag -->
</nav><!--the nav class is closed here  -->
<img src="/images/randombanner.php"/><!--this will get image for the background  -->
...
<section class="right"><!--new section on the right is open here  -->
    ...
    <?php
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) { /* for security */

    if (isset($_POST['submit'])) {

        $stmt = $pdo->prepare('INSERT INTO updates (`date`, title, description)
		  									VALUES (:date, :title, :description)');/* to insert into updates */

        $criteria = [
            'date' => date("Y-m-d H:i:s", strtotime($_POST['date'])), /*for the date  */
            'title' => $_POST['title'], 
            'description' => $_POST['description'], /* for description */
        ];

        $stmt->execute($criteria); /*  to execute the criteria*/

        if ($_FILES['image']['error'] == 0) {
            $fileName = $pdo->lastInsertId() . '.jpg'; /*for image file  */
            move_uploaded_file($_FILES['image']['tmp_name'], '../images/updates/' . $fileName);
        }

        echo 'Update added'; /* after successful update */
    } else { 
        ...
        ?>
			...
			<form action="addupdate.php" method="POST" enctype="multipart/form-data"> <!-- start of  a form-->
				<label>Date</label>
				<input type="date" name="date" /> <!-- for date-->


				<label>Title</label>
				<input type="text" name="title" /> <!--to write the title -->

				<label>Description</label>
				<textarea name="description"></textarea> <!-- to write description of the update-->

				<label>Image</label>

				<input type="file" name="image" /> <!--for the image file -->

				<input type="submit" name="submit" value="Add" /> <!--to submit the filled form  -->
	...
	<form action="index.php" method="post">
				...
				<input type="text" name="username" /> <!--textbox for username -->

				<label>Password</label>
				<input type="password" name="password" /> <!--to key in the password -->
