...
<section class="right"><!--new section on the right is open here  -->
	...
	<?php
		...
		if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) { /*to logon to the website */

			$stmt = $pdo->prepare('DELETE FROM furniture WHERE id = :id'); /* deleting from the table */
			$stmt->execute(['id' => $_POST['id']]); /* executing the post id*/

			echo 'Furniture deleted'; /* to delete the furniture*/
			...
			<input type="text" name="username" /> <!-- username here -->
			<label>Enter Password</label>  
			<input type="password" name="password" /> <!-- password here -->
			<input type="submit" name="submit" value="Log In" />
		</form> <!-- form ended here -->
		<?php
		}
	?>
